...
def preprocess(cfg: DictConfig, name: str) -> Tuple[GlobalConfig, str]:
    ...
    if 'augmentations' in raw_cfg:
        ...
        raw_cfg['augmentations']['before_transform'] = \
            CompositionAugmentation([instantiate(a) for a in raw_cfg['augmentations']['before_transform']])
        # noinspection PyTypeChecker
        raw_cfg['augmentations']['after_transform'] = \
            CompositionAugmentation([instantiate(a) for a in raw_cfg['augmentations']['after_transform']])
