...
class ODEFWithGlobalState(nn.Module):
    """
    Parametrized dynamics function (dz/dt) superclass
    (does not have forward pass defined)
    """
    # noinspection PyMethodMayBeStatic
    def forward_with_grad(self, d: torch.Tensor, g: torch.Tensor, t: torch.Tensor, grad_outputs: torch.Tensor) \
        ...
        """
        batch_size = d.shape[0]  # required for postprocessing

        z_end = self.forward(d, g, t)
        ...
        a_dddz, a_dgdz, a_dfdt, *a_dfdtheta = torch.autograd.grad(
            outputs=(z_end,),  # forward pass outputs
            inputs=(d, g, t) + parameters,  # z, t and theta
        ...
        if a_dddz is None:
            a_dddz = torch.zeros_like(d).to(d)
        if a_dgdz is None:
            a_dddz = torch.zeros_like(g).to(g)
        ...
        if a_dfdtheta is not None:
            ...
            a_dfdtheta = torch.zeros(batch_size, len(parameters)).to(d)

        if a_dfdt is not None:
            a_dfdt = a_dfdt.expand(batch_size, 1) / batch_size
        else:
            a_dfdt = torch.zeros(batch_size, 1).to(d)

        return z_end, a_dddz, a_dgdz, a_dfdt, a_dfdtheta
...
def create_ode_adjoint_func(solver: ODESolver, split_dim: int = -1) -> torch.autograd.Function:
    """
    Creates ODEAdjoint function with chose ODE Solver
    Args:
        solver: ODESolver
        split_dim: Dynamic and global state split dimension
    Returns:
        ODEAdjoint method.
    """
    class ODEAdjoint(torch.autograd.Function):
        ...
        def create_aug_dynamics(batch_size, shape, func) -> Callable:
            ...
            def aug_dynamics(aug_z: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
                ...
                with torch.set_grad_enabled(True):
                    ...
                    d, g = torch.split(z, size, dim=split_dim)
                    z_end, a_dfdz, a_dfdt, a_dfdtheta = func.forward_with_grad(d, g, t, grad_outputs=a)
        ...
        @staticmethod
        def forward(ctx: Any, z0: torch.Tensor, t: torch.Tensor, flat_theta: torch.Tensor, func: ODEFWithGlobalState) -> torch.Tensor:
        ...
        def jvp(ctx: Any, dLdz: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, None]:
            ...
            with torch.no_grad():
                ...
                for i_t in range(time_len-1, 0, -1):
                    ...
                    size = z_i.shape[split_dim] // 2
                    d_i, g_i = torch.split(z_i, size, dim=split_dim)
                    dzdt_i = func(d_i, g_i, t_i).view(batch_size, n_dim)
                ...
                size = z[0].shape[split_dim] // 2
                d_0, g_0 = torch.split(z[0], size, dim=split_dim)
                dzdt_0 = func(d_0, g_0, t[0])
                dLdz_0 = dLdz[0]
...
class NeuralODE(nn.Module):
    ...
    """
    def __init__(self, func: ODEFWithGlobalState, solver: Optional[ODESolver] = None):
        """
        Args:
...
def run_tests() -> None:
    class Linear2dTimeInvariantODEF(ODEFWithGlobalState):
        """
        Simple ODEF for testing
        """
        def __init__(self):
            super().__init__()
            self._linear = nn.Linear(4, 2, bias=False)
